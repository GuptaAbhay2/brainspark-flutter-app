...
class UserState {
  ...
  const UserState({
    ...
    this.avatar = '🧠', this.brainScore = 0,
    this.currentStreak = 0, this.longestStreak = 0,
    this.totalGames = 0, this.isLoading = false, this.error,
  });

  bool get isLoggedIn => userId != null;

  UserState copyWith({
    int? userId, String? username, String? email, String? avatar,
    int? brainScore, int? currentStreak, int? longestStreak,
    int? totalGames, bool? isLoading, String? error,
  }) => UserState(
...
class UserNotifier extends Notifier<UserState> {
  ...
  Future<String?> loginWithPassword(String username, String password,
    ...
    try {
      ...
      final finalScore   = localScore > backendScore ? localScore : backendScore;
  ...
  // === SCORE & STREAK MANAGEMENT (Maths & Score Service ke liye) ===
  ...
  void addScore(int points) {
    ...
    state = state.copyWith(
      brainScore: newScore, totalGames: games,
      currentStreak: newStreak, longestStreak: longest,
    );
  }
  ...
  void updateScore(int newScore, int streak) {
    ...
    box.put('longestStreak', longest);
    state = state.copyWith(brainScore: useScore, currentStreak: streak, longestStreak: longest);
  }
